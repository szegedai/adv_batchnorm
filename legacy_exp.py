import torch
from models.vgg import vgg16, vgg16bn
from models.resnet import resnet110, resnet110f, wide_resnet28x10, wide_resnet28fx10
from models import save_model, load_model, save_checkpoint, load_checkpoint
from attacks import LinfPGDAttack, parameter_presets
...
def main():
    ...
    def objective(params):
        ...
        train_ds, test_ds, _ = load_cifar10(train_transforms, test_transforms, target_transform)
        ...
        model = wide_resnet28x10(10, **CIFAR10_STATS)
        model.to(device)
        ...
        config = {
            ...
            'dataset': 'CIFAR10',
            'training_type': 'adv',
            'architecture': 'WideResNet28x10'
        }
        bias_parameters = [p[1] for p in model.named_parameters() if 'bias' in p[0]]
        scale_parameters = [p[1] for p in model.named_parameters() if 'scale' in p[0]]
        other_parameters = [p[1] for p in model.named_parameters() if not ('bias' in p[0] or 'scale' in p[0])]
        ...
        metrics = train_classifier(model, loss_fn, optimizer, train_loader, test_loader, attack,
                                   ...
                                   callbacks=[
                                       CheckpointCallback(
                                           f'../checkpoints/adv_wresnet28x10_cutmix_{list(params.values())}',
                                           metric_comparator=metrics_comparator
                                       ),
                                       WandBLoggerCallback('AdvBatchNorm', f'adv-wresnet28x10-cutmix-{list(params.values())}', config),
                                       CutMixCallback(1.),
                                       AdversarialPerturbationCallback(attack),
                                       LRSchedulerCallback(scheduler),
                                       EarlyStoppingCallback(3, 0.12)
                                   ],
    ...
    best_value = 0.0
    best_config = None
    for lr in [2e-1, 1e-1, 5e-2, 1e-2, 5e-3, 1e-3, 5e-4, 1e-4]:
